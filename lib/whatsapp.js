...
async function sendInteractiveButtons(to, body, buttons) {
  // buttons: array de strings (id posicional btn_i, legado) o de { id, title } con id semántico
  return _send(to, {
    type: "interactive",
    interactive: {
      type: "button",
      ...
      action: {
        buttons: buttons.map((b, i) => {
          ...
          return {
            type: "reply",
            reply: {
              id: isObj ? b.id : `btn_${i}`,
              title: isObj ? b.title : b,
            },
          };
        }),
      },
...
function parseIncoming(body) {
  try {
    ...
    return {
      ...
      interactiveKind: msg.interactive?.list_reply
        ? "list_reply"
